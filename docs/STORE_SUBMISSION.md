...
- **제품 정의:** 신체 치수 기반 **AI 핏 예측·시각화** 앱(단순 가상 피팅 합성 아님). 사이즈표 자동 분석 × 내 치수 → 부위별 핏(오버/타이트) 예측. 선택적으로 AI 가상 피팅(TryOnCloud) 제공.
- **원격 의존:** CF Worker(`myfit-fashn-proxy.kgg2512.workers.dev`) 1개(AI 피팅 프록시)뿐. 치수·핏 예측·사진 보관은 전부 기기 로컬(localStorage).
...
| 데이터 | 수집? | 용도 | 사용자 연결 | 추적 |
|--------|-------|------|-------------|------|
| **사진(신체 이미지)** | AI 피팅 이용 시에만 | 앱 기능(App Functionality) | 아니오 | 아니오 |
| 신체 치수 | 기기 내 저장(서버 미수집) | 앱 기능 | 아니오 | 아니오 |
| 기기 ID(로컬 저장 키) | 수집 아님(로컬만) | 앱 기능 | 아니오 | 아니오 |

- 핵심 고지: **치수·핏 기록·사진은 기기(localStorage)에만 저장, MyFit 서버로 전송·보관하지 않음.** 사진은 **AI 가상 피팅 이용 시에만** 별도 동의 하에 외부 처리 서버(TryOnCloud)로 전송되고 **처리 즉시 삭제**. 기기 내 재사용 보관 사진은 **7일 자동 폐기**.
...
- 수집/공유 데이터: **사진 및 동영상** — 수집됨(AI 피팅 시), 공유됨(TryOnCloud 처리 목적), 앱 기능용, **전송 중 암호화**, 사용자 삭제 요청 가능(인앱 "내 데이터 전부 삭제").
- 위치/연락처/식별자 추적: **없음**.
...
5. **AI 가상 피팅 벤더 미확정**(TryOnCloud 방향 부적합 검토 중) — 제출 시점에 AI 피팅 기능을 포함할지/치수 핏 예측만 낼지 범위 확정 필요(핏 예측 단독으로도 4.2 방어 가능, §5).
